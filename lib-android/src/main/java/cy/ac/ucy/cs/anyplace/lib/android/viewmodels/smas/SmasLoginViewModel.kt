      ...
      try {
        ...
        if (resp.value is NetworkResult.Error) {
         exception = Exception(resp.value.message)
          ...
          // login failed
          exception = Exception(resp.value.data?.descr ?: "Login failed")
        }
      } catch(e: Exception) {
        exception = e
  ...
  private fun handleResponse(response: Response<SmasLoginResp>):
    ...
    return when {
      ...
      !response.isSuccessful-> {
        val rawErrorResponse = response.errorBody()?.string() ?: "Unknown error"
        val errorReponse = Gson().fromJson(rawErrorResponse, SmasLoginResp::class.java)
        NetworkResult.Error(errorReponse.descr ?: "Unknown error")
      }
      response.isSuccessful -> {
        val user = response.body()
        if (user == null) {
          NetworkResult.Error("Server returned null response")
        } else {
          LOG.D(TAG_METHOD, "LOGIN OK: $user")
          // TODO:DZ SMAS API return 401
          // SMAS API wrongly marks result as OK:
          if (user.status=="err") {
            NetworkResult.Error(user.descr ?: "Login failed")
          } else {
            NetworkResult.Success(user)
          }
        }
      } // can be nullable
      else -> NetworkResult.Error(response.message() ?: "Unknown error")
    }
  }
